...
$id = $_GET['id'];
$res = mysqli_query($conn, "SELECT * FROM trang_place WHERE id = $id");
$data = mysqli_fetch_assoc($res);

// เธชเนเธงเธเธเธฒเธฃเธญเธฑเธเน€เธ”เธ•เธเนเธญเธกเธนเธฅ
if (isset($_POST['update'])) {
    ...
    // เธเธฑเธ”เธเธฒเธฃเนเธเธฅเน (เธ–เนเธฒเธกเธตเธเธฒเธฃเธญเธฑเธเนเธซเธฅเธ”เนเธซเธกเน เนเธซเนเน€เธเธฅเธตเนเธขเธ เธ–เนเธฒเนเธกเน เนเธซเนเนเธเนเธเธทเนเธญเน€เธ”เธดเธก)
    function handleUpdateFile($fileInput, $oldFile) {
        if ($_FILES[$fileInput]['error'] === 0) {
            ...
            move_uploaded_file($_FILES[$fileInput]['tmp_name'], "images/" . $newName);
            return $newName;
        }
        return $oldFile;
    }
    ...
    $docName = handleUpdateFile('filepdf2', $data['filepdf2']);

    $sql = "UPDATE trang_place SET name=?, latitude=?, longitude=?, photo=?, descript=?, typeplace=?, ownplace=?, filepdf2=? WHERE id=?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "ssssssssi", $name, $latitude, $longitude, $photoName, $description, $typeplace, $ownplace, $docName, $id);
    
    if (mysqli_stmt_execute($stmt)) {
        echo "<script>alert('แก้ไขเสร็จแล้ว'); window.location='Read.php';</script>";
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>เนเธเนเนเธเธเนเธญเธกเธนเธฅ</title>
...
<body class="bg-light">
    <div class="container mt-4 mb-5">
        <div class="card shadow">
            <div class="card-header bg-warning text-dark"><h4>ปรับปรุงแก้ไขข้อมูล: <?php echo $data['name']; ?></h4></div>
            <div class="card-body">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label">หน่วยงานรับผิดชอบ: </label>
                            <input type="text" name="ownplace" class="form-control" value="<?php echo $data['ownplace']; ?>">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">แทรกภาพ: <?php echo $data['photo']; ?></label>
                            <input type="file" name="photo" class="form-control" accept="image/*">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">แทรกไฟล์: <?php echo $data['filepdf2']; ?></label>
                            <input type="file" name="filepdf2" class="form-control" accept=".pdf,.docx">
                        </div>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">สังเขป</label>
                        <textarea name="description" class="form-control" rows="3"><?php echo $data['descript']; ?></textarea>
                    </div>

                    <div class="d-flex gap-2">
                        <button type="submit" name="update" class="btn btn-success flex-grow-1">บันทึกข้อมูล</button>
    ...
    <script>
        function initMap() {
            ...
            // เน€เธกเธทเนเธญเธเธฅเธดเธเนเธเธเธ—เธตเนเน€เธเธทเนเธญเธขเนเธฒเธขเธซเธกเธธเธ”
